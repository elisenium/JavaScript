...
module.exports.listByHClass = (hClass) => {
  const stmt = db.prepare('SELECT * FROM EXOPLANETS WHERE hclass LIKE ?');
  return stmt.all(hClass + '%');
};

module.exports.deleteAll = () => {
  const info = db.prepare('DELETE FROM EXOPLANETS').run();
  console.log("exoplanet model delete EVERYTHING" + info.changes);
};